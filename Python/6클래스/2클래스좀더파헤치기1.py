...
# 클래스 내부에 __init__(생성자) 라는 함수를 만들면 객체를 생성할때 처리할 내용을 작성할 수 있다.
# 문법
# class 클래스이름:
...
class Dog: # 클래스 선언
    ...
    # 설명 : 매개변수 self에는 Dog클래스로 부터 생성한 자기자신의 Dog객체를 전달받아 사용한다.
...
my_dog = Dog("삼식이") # 인스턴스 생성(객체 생성)시 매개변수가 하나의 생성자를 호출하여
                      # name 변수값을 초기화 함
...
#  이에 따라 다음과 같이 구분 할 수 있습니다

    #1. 클래스 변수
    #  - 해당 클래스에서 생성된 모든 인스턴스가 값을 공유하는 변수
    #2. 인스턴스 변수
    # - __init__ 생성자 내에서 선언된 변수로
    # 인스턴스가 생성 될 때 마다 새로운 값이 할당되는 변수 입니다.
